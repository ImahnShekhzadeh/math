...
\begin{remark}
	Instead of defining a topology via open sets, it is alternatively possible to use closed sets. Then the above axioms become: 
	\begin{itemize}
		\item the union of two closed sets in $\tau$ is closed,
		\item the intersection of any collection of closed sets in $\tau$ is closed,
		\item $\emptyset, X\in \tau$.
	\end{itemize}
\end{remark}

\begin{proof}
	Let $I$ be an index set and $\{U_i\}_{i\in I}$ a family of open subsets of $X$, i.e. $\{X\setminus U_i\}_{i\in I}$ is a family of closed subsets of $X$. Then
	\[
		\bigcap_{i\in I} X\setminus U_i \overset{\tiny\eqref{eq:de_morgan_comp_union}}{=} X\setminus \bigcup_{i\in I} U_i,
	\]
	is closed, as $\bigcup_{i\in I}U_i$ is open. 
	
	Let $U_1, U_2$ be open sets, then
	\[
		\bigcup_{i\in \{1, 2\}}X\setminus U_i \overset{\tiny\eqref{eq:de_morgan_comp_intersec}}{=} X\setminus\bigcap_{i\in \{1, 2\}}U_i 
	\]
	is closed, as $\bigcap_{i\in \{1, 2\}}U_i$ is open.
	
	Since $\emptyset$ and $X$ are open, $X\setminus \emptyset = X$ and $X\setminus X = \emptyset$ are closed. 
\end{proof}
...
\begin{theorem}\label{thrm:metric_is_continuous_function}
...
\begin{proof}[Proof \cite{4561719,4561736,4203835}]
...
\begin{theorem}\label{thrm:product_comp_spaces}
...
\begin{exmp}
	From the Heine-Borel Theorem \ref{thrm:heine-borel}, we already know that $I^1 = [0, 1]$ is compact. The preceding corollary implies that for any $n\in\mathbb N$, the unit $n$-cube $I^n := \prod_{i=1}^{n}I^1$ is also compact.
\end{exmp}

\begin{theorem}\label{thrm:closed_subset_comp_top_space}
...
\begin{theorem}[Generalized Heine-Borel Theorem]\label{thrm:generalized_heine_borel}
	A subset $A\subset \mathbb R^n$ is compact iff it is closed and bounded.
\end{theorem}

\begin{proof}[Proof \cite{topology-singh}]
	\enquote{$\Longleftarrow$} Let $A\subset\mathbb R^n$ be closed and bounded. For each $i\in \{1, \dots, n\}$, let $p_i: \mathbb R^n\to\mathbb R$ denote the projection onto the $i$th factor. Then $p_i(A)\subset\mathbb R$ is bounded. To see this, note that since $A$ is bounded, there is an $r > 0$ and a $\varphi = (\varphi_1, \dots, \varphi_n)\in\mathbb R^n$ s.t. $A\subset B_{r}[\varphi] = \left\{\psi = (\psi_1, \dots, \psi_n)\in\mathbb R^n\mid \sqrt{\sum_{i=1}^{n}(\varphi_i - \psi_i)^2} \leq r\right\}$, where we used the fact that the product topology on $\mathbb R^n$ is induced by the Euclidean metric. Also, for each $i\in \{1, \dots, n\}$,
	\begin{align*}
		p_i(A) &= \{p_i(x) \mid x = (x_1, \dots, x_n)\in A\}
		\\ & = \{x_i \mid x\in A\} 
		\\ &\subset \left\{x_i \mid x\in B_{r}[\varphi]\right\} 
		\\ &= \left\{x_i \mid \sqrt{\sum_{i=1}^{n}(\varphi_i - x_i)^2}\leq r\right\}
		\\ &\subset B_{r}[\varphi_i] = \{y\in \mathbb R \mid \abs{y - \varphi_i} \leq r\},
	\end{align*}
	since $\abs{x - \varphi_i} = (x - \varphi_i)^2 \leq \sqrt{\sum_{i=1}^{n}(\varphi_i - x)^2} \leq r$. Thus, $p_{i}(A)$ is bounded as well. Defining $a_i := \varphi_i - r$ and $b_i := \varphi_i + r$, we have $p_i(A) \subset [a_i, b_i]$. It now holds that $A\subset \prod_{i=1}^{n}p_i(A)$, since for any $x = (x_1, \dots, x_n)\in A$, we have $x_i\in p_i(A)$ for each $i$ in $\{1, \dots, n\}$. Thus, $A \subset \prod_{i=1}^{n}p_i(A) \subset \prod_{i=1}^{n}[a_i, b_i] =: X$. By Theorem \ref{thrm:heine-borel} and \ref{thrm:product_comp_spaces}, $X$ is compact.
	
	We now show that $A$ is also closed in $X$, equipped with the subspace topology. Since $A\subset\mathbb R^n$ is closed, $\mathbb R^n\setminus A$ is open. Thus, $X\cap (\mathbb R^n\setminus A)$ is open in the subspace topology of $X$. Since $X \cap (\mathbb R^n\setminus A) = X\setminus A$, $A\subset X$ is closed. To see why $X \cap (\mathbb R^n\setminus A) = X\setminus A$, let $x\in X \cap (\mathbb R^n\setminus A)$, i.e. $x\in X$ and $x\in\mathbb R^n$ and $x\notin A$, which implies $x\in X\setminus A$.
	
	Since $A\subset X$ is closed and $X$ is compact, by Theorem \ref{thrm:closed_subset_comp_top_space}, $A$ is compact. 
	
	\enquote{$\Longrightarrow$} Let $A\subset\mathbb R^n$ be compact. Since $\mathbb R^n$ is a Hausdorff space, $A$ is closed, cf. Theorem \ref{thrm:compact_subspace_Hausdorff_closed}. Since $A$ is compact, every open cover of $A$ contains a finite sub-covering. Fix an $a\in A$ and consider the open covering $\{B_{m}(a) \mid m\in\mathbb N\}$ of $A$, i.e. the open covering consists of open balls around $a$ with varying radii $m\in\mathbb N$. Since $A$ is compact, there is a finite sub-covering, and since $B_{m}(a) \cup B_{n}(a) = B_{n}(a)$ if $n \geq m$, there is an $N\in\mathbb N$ s.t. $A\subset B_{N}(a)\subset B_{N}[a]$, showing that $A$ is bounded. 
\end{proof}
...
\begin{theorem}\label{thrm:compact_subspace_Hausdorff_closed}
...
\begin{theorem}
	Let $(X, \tau_X)$ and $(Y, \tau_Y)$ be topological spaces and let $A\subset X$ be equipped with the subspace topology. If $f: X\to Y$ is continuous, then so is $f\vert_{A}: A\to Y$.
\end{theorem}

\begin{proof}[Proof \cite{1826827}]
	Let $U\in \tau_Y$, then 
	$$f\vert_{A}^{-1}(U) = \{a\in A \mid f(a)\in U\} = \{x\in X\mid f(x)\in U\} \cap A = f^{-1}(U) \cap A.$$ 
	Since $f: X\to Y$ is continuous, this implies that $f^{-1}(U)$ is open in $X$ and thus $f^{-1}(U) \cap A$ is open in the subspace topology of $A$.
\end{proof}
...
\begin{corollary}\label{corollary:continuous_real_valued_function_attains_min_max}
	Let $X$ be a compact topological space, equip $\mathbb R$ with the standard topology and let $f: X\to\mathbb R$ be continuous. Then $f$ attains its minimum and maximum on $X$.
\end{corollary}

\begin{proof}[Proof \cite{109552}]
	Since $f$ is continuous and $X$ compact, $f(X)\subset \mathbb R$ is compact, and thus by Theorem \ref{thrm:generalized_heine_borel} closed and bounded. Corollary \ref{corollary:closed_bounded_subsets_contain_inf_sup} now implies that $\sup(f(X)), \inf(f(x))\in f(X)$ and $\sup(f(X)) = \max(f(X))$ as well as $\inf(f(X)) = \min(f(X))$. Thus, there exist $x_0, x_1\in X$ s.t. $\min(f(X)) = f(x_0)$ and $\max(f(X)) = f(x_1)$.
\end{proof}
