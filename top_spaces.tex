...
\begin{defn}[Neighborhood \cite{topology-singh}]
	Let $(X, \tau)$ be a topological space and $x\in X$, then a set $N\subset X$ is called a \textit{neighborhood} of $x$ in $X$ if there is an open set $U\in\tau$ with $x\in U\subset N$. If $N$ is open, then we say that $U$ is an \textit{open neighborhood} of $x$ in $X$. 
\end{defn}
