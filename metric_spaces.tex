...
\begin{exmp}\label{exmp:closed-set-singleton-R}
	Let $X = \mathbb R$ with the standard metric. Then for each $x\in \mathbb R$, the singleton $\{x\}\subset\mathbb R$ is closed.
\end{exmp}

\begin{proof}
	Note that $\mathbb R\setminus \{x\} = (-\infty, x) \cup (x, \infty)$ is open.
\end{proof}
...
\begin{proof}
	An illustration of the idea of the proof is shown in Figure \ref{fig:open-ball-contains-open-ball}.
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.35\textwidth]{Figures/open-balls-open.png}
		\caption{In a metric space, each open ball contains another open ball of smaller radius.}
		\label{fig:open-ball-contains-open-ball}
	\end{figure}
...
\begin{proof}
	...
	Define $N := \max\left\{N', \tilde{N}\right\}$, then for all $n\geq N$, we have
...
\begin{defn}[Pointwise convergence \cite{src:pointwise_uniform_convergence}]
...
\begin{defn}[Uniform convergence \cite{src:pointwise_uniform_convergence}]
...
\begin{theorem}
	Let $U\subset X$. Then its closure, $\overline{U}$, is the \textit{smallest} closed set in $X$ containing $U$, i.e. 
	\begin{align}\label{eq:closure}
		\overline{U} = \bigcap \left\{ G \mid G \text{ is closed in } X \text{ and } U\subset G \right\}.
	\end{align} 
\end{theorem}
...
\begin{defn}[Interior \cite{topology-singh}]\label{defn:interior}
...
\begin{theorem}\label{thrm:closure_interior_relation}
	Let $X$ be a metric space and $U\subset X$. Then
	\begin{align}
		X\setminus\inte{U} = \overline{X\setminus U}.
	\end{align}
\end{theorem}

\begin{proof}[Proof \cite{970841}]
	We use the definitions and make use of de Morgan's law.
	\begin{align*}
		X\setminus \inte{U} &\overset{\tiny\eqref{eq:interior_complement}}{=} X\setminus \bigcup \left\{G\mid G \text{ is open in } X \text{ and } G\subset U\right\} \\ &\overset{\tiny\eqref{eq:de_morgan_comp_union}}{=} \bigcap \left\{ X\setminus G \mid X\setminus G \text{ is closed in } X \text{ and } X\setminus U\subset X\setminus G \right\}
		\\ &\overset{\tiny\eqref{eq:closure}}{=} \overline{X\setminus U}.
	\end{align*}
\end{proof}
...
\begin{exmp}[Dense subsets]\label{exmp:irrationals-dense}
...
\begin{proof}[Proof \cite{2497036}]
	\enquote{$\Longrightarrow$} Use Theorem \ref{thrm:property_metric_space} and note that $U\subset X$ is dense, i.e. $\overline{U} = X$.
	
	\enquote{$\Longleftarrow$} We will prove the contraposition, i.e. assume $U\subset X$ is not dense, then $\overline{U}\ne X$, which implies $O := X\setminus\overline{U} \ne \emptyset$, i.e. $O \cap \overline{U} = \emptyset$. Since $U\subset\overline{U}$, $O\cap U = \emptyset$. Clearly, $O$ is open, since it is the complement of a closed set. 
\end{proof}

\begin{theorem}\label{thrm:dense_interior}
...
\begin{theorem}\label{thrm:finite_intersec_open_dense}
	Let $X$ be a metric space and let $U_1, U_2\subset X$ be open and dense. Then $U_1\cap U_2$ is also open and dense.
\end{theorem}

\begin{proof}[Proof \cite{1143218}]
	Since $U_1$ is dense, by Theorem \ref{thrm:characterization-dense-subsets}, for every non-empty open set $O\subset X$, we have $U_1\cap O\ne \emptyset$. Since both $U_1$ and $O$ are open, so is $U_1\cap O$. Thus, since $U_2$ is also dense, $U_2\cap U_1\cap O\ne \emptyset$, which shows that $U_1\cap U_2$ intersects every non-empty open set $O\subset X$. Finally, $U_1\cap U_2$ is also open, as it is the finite intersection of two open sets.
\end{proof}

\begin{remark}
	Let $U_1, U_2\subset X$ be dense. As the proof in the preceding Theorem shows, it is only necessary that either $U_1$ or $U_2$ is open for $U_1 \cap U_2$ to be dense. However, it is necessary that either $U_1$ or $U_2$ is open. If neither $U_1$ or $U_2$ is open, the resulting intersection may not be dense.
\end{remark}

\begin{exmp}[Proof \cite{1506081}]
	Let $X = \mathbb R$ with the standard metric. Consider the rationals $\mathbb Q$ and irrationals $\mathbb R\setminus \mathbb Q$. Neither set is open in $\mathbb R$. Their intersection is the empty set, which is clearly not dense in $\mathbb R$.
\end{exmp}
...
\begin{theorem}[Arzel\`a\footnote{\textsc{Cesare Arzel\`a} (1847 -- 1912), Italian mathematician.}-Ascoli\footnote{\textsc{Giulio Ascoli} (1843 -- 1896), Jewish-Italian mathematician.}]\label{thrm:arzela-ascoli}
...
\begin{proof}[Proof \cite{src:arzela_ascoli_theorem}]
...
In Theorem \ref{thrm:finite_intersec_open_dense}, we have already established that the finite intersection of open dense sets yields an open dense set again. While in the case of infinite intersections, it is inevitable that we lose the openness property of the resulting intersection, in case of countable intersections, it turns out that we still have a dense set, as long as we assume that the underlying metric space is complete.

Before stating and proving the Baire Category Theorem, we will prove Cantor's intersection theorem, which we will use for the proof of the Baire Category Theorem.

\begin{theorem}[Cantor's intersection theorem]\label{thrm:Cantor_intersection_theorem}
	Let $X$ be a complete metric space, and let $\seq[V_n]$ be a decreasing nested sequence of non-empty closed subsets of $X$, i.e. $V_{n + 1}\subset V_n$ for all $n\geq 1$, with vanishing diameter, i.e. $\text{diam}(V_n)\to 0$ for $n\to\infty$. Then there is an $x\in X$ s.t. 
	\[
		\{x\} = \bigcap_{n\in\mathbb N}V_n.
	\]
\end{theorem}

\begin{proof}[Proof \cite{src:baire_category_theorem}]
	In each set $V_n$, choose an $x_n$. Note that the sequence $\seq[x_n]$ is Cauchy, since for arbitrary $N\in\mathbb N$, if $m, n\geq N$, we have $d(x_m, x_n) \leq \text{diam}(V_N)$, as $x_m\in V_m\subset V_N$ and $x_n\in V_n\subset V_N$. By assumption, $\seq[\text{diam}(V_n)]$ converges to $0$, i.e. for all $\epsilon > 0$ there exists an $N = N(\epsilon)$ s.t. $\abs{\text{diam}(V_n)} = \text{diam}(V_n) < \epsilon$ for all $n\geq N$. Now, for an $\epsilon > 0$, choose the same $N$ as given by the convergence of $\seq[\text{diam}(V_n)]$, then $d(x_m, x_n) \leq \text{diam}(V_{N}) < \epsilon$ for all $m, n\geq N$. Since $X$ is complete, the Cauchy sequence $\seq[x_n]$ converges to an $x\in X$.
	
	Now fix $M\in\mathbb N$. Then for all $n\geq M$, $x_n\in V_n\subset V_M$. This implies that $x$ is an accumulation point of $V_M$. As $V_M$ is closed, $x\in V_M$. Since $M\in\mathbb N$ was arbitrary, this implies that $x\in \bigcap_{M\in\mathbb N}V_M$. 
	
	Assume that for $y\in X$, $y\in \bigcap_{M\in\mathbb N}V_M$, i.e. $x, y\in V_n$ for all $n\in\mathbb N$. For all $n$,
	\[
		d(x, y) \leq d(x, x_n) + d(x_n, y) \leq 2\cdot\text{diam}(V_n)\overset{n\to\infty}{\to}0,
	\]
	i.e. $x = y$. Thus, 
	\[
		\{x\} = \bigcap_{n\in\mathbb N}V_n.
...
\begin{theorem}[Baire Category Theorem]\label{thrm:baire_category_theorem_v1}
	Let $X$ be a metric space and $\seq[U_n]$ a sequence of open and dense subsets of $X$. Then $\bigcap_{n\in\mathbb N}U_n$ is also dense in $X$.
\end{theorem}

\begin{proof}[Proof \cite{src:baire_category_theorem}]
	Let $\seq[U_n]$ be a sequence of open and dense subsets of $X$ and let $V\subset X$ be open and non-empty. Define the sequence $\seq[a_n] := \seq[1/n]$. We now inductively construct sequences $\seq[\varphi_n]$ in $X$ and $\seq[r_n]$ in $\mathbb R$ s.t. 
	\begin{enumerate}[label=\alph*)]
		\item $r_n < a_n$ for all $n\in\mathbb N$ and
		\item $B_{r_{n}}[\varphi_{n}] \subset B_{r_{n - 1}}(\varphi_{n - 1}) \cap U_{n}$ for all $n\in\mathbb N\setminus \{1\}$.
	\end{enumerate}
	{ \small
		\textbf{Base case}: For $n = 1$, by Theorem \ref{thrm:characterization-dense-subsets}, we know that $V\cap U_1$ is non-empty. Let $\varphi_1\in V\cap U_1$. Then there exists an $\tilde{r}_1 > 0$ s.t. $B_{\tilde{r}_1}(\varphi_1)\subset V\cap U_1$. Now define $r_1 := \frac{1}{2}\min\{a_1, \tilde{r}_1\} < a_1$.
		
		$n = 2$: Since $B_{r_1}(\varphi_1)$ is open and non-empty, by Theorem \ref{thrm:characterization-dense-subsets}, $B_{r_1}(\varphi_1)\cap U_2$ is non-empty. Let $\varphi_2\in B_{r_1}(\varphi_1)\cap U_2$. Then there exists an $\tilde{r}_2 > 0$ s.t. $B_{\tilde{r}_2}(\varphi_2)\subset B_{r_1}(\varphi_1)\cap U_2$. Now define $r_2 := \frac{1}{2}\min\{a_2, \tilde{r}_2\} < a_2$ and note that 
		\[
			B_{r_2}[\varphi_2] \subset B_{2r_2}(\varphi_2) \subset B_{\tilde{r}_2}(\varphi_2)\subset B_{r_1}(\varphi_1)\cap U_2. {\quad\color{green}\Large\checkmark}
		\]
		\textbf{Induction hypothesis}: Assume that for an $n\in\mathbb N\setminus\{1\}$, there exists an $r_{n}\in\mathbb R$ s.t. $ 0 < r_{n} < a_{n}$ and a $\varphi_{n}\in X$ s.t. $B_{r_{n}}[\varphi_{n}] \subset B_{r_{n - 1}}(\varphi_{n - 1}) \cap U_{n}$. 
		\\[5pt]
		\textbf{Induction step}: Since $B_{r_n}(\varphi_n)$ is open and non-empty, by Theorem \ref{thrm:characterization-dense-subsets}, $B_{r_n}(\varphi_n) \cap U_{n + 1}$ is non-empty. Let $\varphi_{n + 1}\in B_{r_n}(\varphi_n) \cap U_{n + 1}$. Then there exists an $\tilde{r}_{n + 1} > 0$ s.t. $B_{\tilde{r}_{n + 1}}(\varphi_{n + 1}) \subset B_{r_n}(\varphi_n) \cap U_{n + 1}$. Now define $r_{n + 1} := \frac{1}{2}\min\{a_{n + 1}, \tilde{r}_{n + 1}\} < a_{n + 1}$ and note that 
		\[
			B_{r_{n + 1}}[\varphi_{n + 1}] \subset B_{2r_{n + 1}}(\varphi_{n + 1}) \subset B_{\tilde{r}_{n + 1}}(\varphi_{n + 1}) \subset B_{r_n}(\varphi_n) \cap U_{n + 1}. {\quad\color{green}\Large\checkmark}
		\]
	}
	This way, we have constructed a decreasing nested sequence $\left(B_{r_n}[\varphi_n]\right)_{n\in\mathbb N}$ of non-empty closed subsets of $X$. The sequence is nested, as for all $n\in\mathbb N\setminus\{1\}$,
	\[
		B_{r_n}[\varphi_n] \subset B_{r_{n - 1}}(\varphi_{n - 1}) \cap U_n \subset B_{r_{n - 1}}(\varphi_{n - 1}) \subset B_{r_{n - 1}}[\varphi_{n - 1}].
	\]
	Thus, by Theorem \ref{thrm:Cantor_intersection_theorem}, $\bigcap_{n\in\mathbb N}B_{r_n}[\varphi_n]$ is non-empty. Since for any $n\in\mathbb N\setminus\{1\}$,
	\begin{align*}
		B_{r_n}[\varphi_n] &\subset B_{r_{n - 1}}(\varphi_{n - 1}) \cap U_n 
		\\ &\subset B_{r_{n - 1}}[\varphi_{n - 1}]\cap U_n 
		\\ &\subset \cdots \subset B_{r_{1}}(\varphi_1) \cap U_n 
		\\ &\subset 
		B_{\tilde{r}_1}(\varphi_1) \cap U_n 
		\\ &\subset V\cap U_n.
	\end{align*} 
	For $n = 1$, note that $B_{r_1}[\varphi_1] \subset B_{\tilde{r}_1}(\varphi_1) \subset V \cap U_1$. Thus, 
	\[
		\{x\} = \bigcap_{n\in\mathbb N}B_{r_n}[\varphi_n] \subset \bigcap_{n\in\mathbb N}V\cap U_n = V\cap \bigcap_{n\in\mathbb N}U_n,
	\]
	As $V$ is an arbitrary, non-empty open subset of $X$, by Theorem \ref{thrm:characterization-dense-subsets}, $\bigcap_{n\in\mathbb N}U_n$ is dense.
\end{proof}

In the literature, one often finds the following version stated as the Baire Category Theorem 

\begin{theorem}[Baire Category Theorem]\label{thrm:baire_category_theorem_v2}
	Let $X$ be a metric space and $\seq[U_n]$ a sequence of closed subsets of $X$ such that their union $\bigcup_{n\in\mathbb N}U_n$ contains an open ball. Then there is a $U_n$ that contains an open ball.
\end{theorem}

\begin{remark}
	The versions of the Baire Category Theorem stated in the preceding two theorems are equivalent.
\end{remark}

\begin{proof}[Proof \cite{3558397, 5085233}]
	\enquote{$\Longrightarrow$} Let $\seq[U_n]$ be a sequence of closed subsets of $X$ s.t. their union contains an open ball. We now want to show that there is a $U_n$ containing an open ball. Assume that no $U_n$ contains an open ball, then the interior of every $U_n$ is empty, i.e. by Theorem \ref{thrm:dense_interior}, every $X\setminus U_n$ is dense. Since $\seq[X\setminus U_n]$ is a sequence of open and dense subsets of $X$, by Theorem \ref{thrm:baire_category_theorem_v1}, $\bigcap_{n\in\mathbb N}X\setminus U_n = X\setminus\bigcup_{n\in\mathbb N}U_n$ is dense, implying that $\inte{\bigcup_{n\in\mathbb N}U_n} = \emptyset$, see Theorem \ref{thrm:dense_interior}. This is a contradiction to $\bigcup_{n\in\mathbb N}U_n$ containing an open ball.
	
	\enquote{$\Longleftarrow$} Let $\seq[U_n]$ be a sequence of open and dense subsets of $X$. We want to show that $\bigcap_{n\in\mathbb N}U_n$ is dense, which we will do via contradiction. So let us assume that $\bigcap_{n\in\mathbb N}U_n$ is not dense. Then by Theorem \ref{thrm:dense_interior}, \[
		\inte{X\setminus \bigcap_{n\in\mathbb N}U_n} = \inte{\bigcup_{n\in\mathbb N}X\setminus U_n} \ne\emptyset. 
	\]
	Thus, by definition, $\bigcup_{n\in\mathbb N}X\setminus U_n$ contains an open set in $X$ and hence an open ball. Since $\seq[X\setminus U_n]$ is a sequence of closed sets whose union contains an open ball, by Theorem \ref{thrm:baire_category_theorem_v2}, there is an $n_0\in\mathbb N$ s.t. $X\setminus U_{n_0}$ contains an open ball, which implies that $\inte{X\setminus U_{n_0}}\ne\emptyset$, and by Theorem \ref{thrm:dense_interior} that $U_{n_0}$ is not dense, which is a contradiction to the assumption that all $U_n$ are dense. 
\end{proof}

There are many applications of the Baire Category Theorem, cf. Ref. \cite{165696} for some. One application is to show that the irrationals $\mathbb R\setminus\mathbb Q$ are dense. In Example \ref{exmp:irrationals-dense}, another proof of this statement can be found.

\begin{exmp}
	Let $X=\mathbb R$ with the standard metric. According to Example \ref{exmp:closed-set-singleton-R}, for any $q\in\mathbb Q$, the singleton $\{q\}$ is closed in $\mathbb R$, i.e. $\mathbb R\setminus \{q\}$ is open. $\mathbb R\setminus \{q\}$ is also dense, as by Theorem \ref{thrm:closure_interior_relation}, $\overline{\mathbb R\setminus\{q\}} = \mathbb R\setminus \inte{\{q\}} = \mathbb R\setminus\emptyset = \mathbb R$. Thus, by Eq. \eqref{eq:de_morgan_comp_union}, $\bigcap_{q\in\mathbb Q}\mathbb R\setminus \{q\} = \mathbb R\setminus \bigcup_{q\in\mathbb Q}\{q\} = \mathbb R\setminus \mathbb Q$. By the Baire Category Theorem, we know that $\mathbb R\setminus \mathbb Q$ is dense.
\end{exmp}

For uncountable intersections of open and dense sets of a complete metric space, the resulting set is not dense, as the following example shows \cite{1070418}. 

\begin{exmp}
	Let $X = \mathbb R$ with the standard metric. For any $x\in\mathbb R$, the singleton $\{x\}$ is closed, i.e. $\mathbb R\setminus \{x\}$ is open. $\mathbb R\setminus \{x\}$ is also dense. However, by Eq. \eqref{eq:de_morgan_comp_union}, $\bigcap_{x\in \mathbb R}\mathbb R\setminus \{x\} = \mathbb R \setminus \bigcup_{x\in\mathbb R}\{x\} = \emptyset$, which is not dense in $\mathbb R$.
\end{exmp}

Note that in the formulation of the Baire Category Theorem, it is vital that the underlying metric space is complete. Otherwise, the Baire Category Theorem may not hold, as the following example shows \cite{2369429}.

\begin{exmp}
	Let $X = \mathbb Q$ with its metric inherited from $\mathbb R$. Then for any $q\in\mathbb Q$, $\mathbb Q\setminus \{q\}$ is open and dense, since $\{q\}$ is closed and by Theorem \ref{thrm:closure_interior_relation}, $\overline{\mathbb Q\setminus \{q\}} = \mathbb Q\setminus\inte{\{q\}} = \mathbb Q\setminus \emptyset = \mathbb Q$. However, 
	\[
		\bigcap_{q\in\mathbb Q}\mathbb Q\setminus \{q\} = \mathbb Q\setminus \bigcup_{q\in\mathbb Q}\{q\} = \emptyset,
	\]
	which is clearly not dense in $\mathbb Q$.
\end{exmp}

When it comes to the unions of open and dense sets, the requirements for the union to be dense are much weaker, as the following Theorem shows. 

\begin{theorem}
	Let $X$ be a metric space, $I$ an index set and $\left\{U_{i}\right\}_{i\in I}$ a family of subsets of $X$. If one $U_i$ is dense, then their union $\bigcup_{i\in I}U_i$ is dense in $X$. If all $U_i$ are open, then their union is open as well.
\end{theorem}

\begin{proof}[Proof \cite{2704447}]
	Let $i_0\in I$ be such that $U_{i_0}\subset X$ is dense, i.e. $\overline{U_{i_0}} = X$. Since $U_{i_0}\subset\bigcup_{i\in I}U_i$, this implies that 
	\[
		\overline{U_{i_0}} = X \subset \overline{\bigcup_{i\in I}U_{i}}\subset X,
	\]
	see Corollary \ref{corollary:closure_subset_property}. Thus, $\bigcup_{i\in I}U_{i}$ is dense.
\end{proof}
