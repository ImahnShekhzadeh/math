...
\begin{defn}[Open Covering]
	A \textit{covering} of a set $X$ is a family $\mathscr G$ of subsets of $X$ s.t. $$X \subset \bigcup\{G\mid G\in\mathscr G\}.$$
	We also say that $\mathscr G$ covers $X$. If $X$ is a topological space, cf. Definition \ref{defn:topological_space}, and every member of $\mathscr G$ is open, then $\mathscr G$ is called an \textit{open covering} of $X$.
\end{defn}
...
\begin{exmp}
	Consider $U := (0, 1]$, which is a subset of the topological space $R$ with the standard topology on $\mathbb R$. Then $V_j := (1/j, 1 + 1/j)$, $j\in\mathbb N$, are an open covering of $U$, but $U$ is not compact, since there is no \textit{finite} covering of $U$ with open sets from from $\{V_j\}_{j\in\mathbb N}$.
