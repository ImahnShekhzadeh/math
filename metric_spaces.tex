...
\begin{remark}
	...
	\]
	Clearly, each $f_n\in\mathscr F$ is uniformly continuous, since the domain $[0, 1]$ is compact and $f_n$ is continuous on $[0, 1]$, as it is the product of two continuous functions on $[0, 1]$ and the denominator is bigger than $0$ for all $n\in\mathbb N$ and $x\in [0, 1]$. However, $\mathscr F$ is not uniformly equicontinuous. Let $\epsilon = 0$, then for all $\delta > 0$, take $p = 0$ and $z = 1/n$. There exists an $n\in\mathbb N$ s.t. $\abs{p - z} = \frac{1}{n} < \delta$, yet
	\[
