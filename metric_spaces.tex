...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item triangle inequality: Let $\varphi, \psi, \chi\in \mathcal C[a, b]$, then by Theorem \ref{thrm:sup_inequality}, \begin{align*}
			d_{\infty}(\varphi, \psi) &= \sup_{x\in [a, b]}\left\{ \abs{\varphi(x) - \psi(x)} \right\} \leq \sup_{x\in [a, b]}\{\abs{\varphi(x) - \chi(x)}\} 
			+ \sup_{x\in[a, b]}\{\abs{\chi(x) - \psi(x)}\} \\ &= d_{\infty}(\varphi, \chi) + d_{\infty}(\chi, \psi),
		\end{align*}
		since for any $f\leq g$, $\sup f\leq \sup g$.
	\end{enumerate}
	
	We now show that $(\mathcal C[a, b], d_{\infty})$ is complete. Let $(\varphi_n)_{n\in\mathbb N}$ be a Cauchy sequence in $\mathcal C[a, b]$, then for any $x\in [a, b]$:
	\begin{align}
