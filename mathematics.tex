...
\title{Mathematics}
% \subtitle{Questions}
\author{Imahn Shekhzadeh\footnote{Computer Science Department, University of Geneva, Route de Drize 7, Carouge, Switzerland} \\ \color{blue}imahn.shekhzadeh@unige.ch\color{black}\normalsize}
\date{\today}
...
\begin{document}
	...
	\newpage 
	\section{Topology}
	\begin{defn}[Metric Space \cite{fa2019}]
	...
	\end{align}
	i.e. $d(f, \psi) < r$ and thus $\psi\in B_{r}(f)$.  \qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\quad$\blacksquare$
	
	\begin{theorem}
		Let $(X, d)$ be a metric space. The collection of open sets as in Definition \ref{defn-open-set} gives a topology. Thus, every metric space is a topological space. 
	\end{theorem}
