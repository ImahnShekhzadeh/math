...
\begin{document}
	...
	\begin{remark}
		There exist metrics $d$ and $d'$ on a set $X$ that are (topologically) equivalent, but not strongly equivalent.
	\end{remark}
	...
	\begin{theorem}
		Let $(X, d)$ and $(X', d')$ be metric spaces and $f: X\rightarrow X'$  be an isometry. If $f(\varphi) = f(\psi)$ for any $\varphi$, $\psi\in X$, then 
		\begin{align}
	...
	\begin{remark}
		Two isometric spaces $(X, d)$ and $(X', d')$ do not differ in their properties wrt their metrics, but instead through the properties of their individual elements. Any metric property holding in one metric space also holds in the other. Hence, in this sense, we say that $(X, d)$ and $(X', d')$ are \textit{identical}.
	\end{remark}
	...
	\begin{remark}
		...
		\end{align}
		where all elements are in $\mathbb Q$, does not converge in $\mathbb Q$ (the limit point is Euler's number $e$).
	\end{remark}
