...
\begin{document}
	...
	\section{Topological Spaces}
		...
		\begin{exmp}
			Consider the function $f: [0, 2\pi)\to \mathbb S^1, t\mapsto e^{it}$, where $$\mathbb S^1 := \left\{e^{it} \mid t\in [0, 2\pi)\right\} = \left\{(x, y)\in\mathbb R^2 \mid x^2 + y^2  = 1\right\}$$ is the unit circle in $\mathbb R^2$. Let $[0, 2\pi)\subset\mathbb R$ and $\mathbb S^1\subset\mathbb R^2$ be equipped with the subspace topologies, cf. Theorem \ref{thrm:subspace_topology}, where the topology on $\mathbb R$ and $\mathbb R^2$ is the Euclidean topology.
		\end{exmp}
		...
		\begin{proof}
			...
			Let $G\subset \mathbb S^1$ be an open arc that does not contain $1\in \mathbb S^1$, then $f^{-1}(G) = (a, b)$, where $0 < a < b < 2\pi$. If $G$ does contain $1\in\mathbb S^1$, then $f^{-1}(G) = [0, a) \cup (b, 2\pi)$, where $0 < a < b < 2\pi$. Thus, regardless of whether $G$ contains $1$, $f^{-1}(G)$ is open in $[0, 2\pi)$ -- note that $f^{-1}(G)$ is open in $\mathbb R$ -- and thus $f$ is continuous by Theorem \ref{thrm:continuity_sub_basis}.
		\end{proof}
