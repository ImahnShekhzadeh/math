...
\begin{document}
	...
	\section{Topological Spaces}
		...
		\begin{defn}[Subbasis \cite{topology-singh}]
			Let $(X, \tau)$ be a topological space, then a \textit{subbasis} for $\tau$ is a collection of subsets $\mathscr S\subset \mathscr P(X)$ s.t. $\tau = \tau(\mathscr S)$.
		\end{defn}
		
		\begin{remark}
			Let $(X, \tau)$ be a topological space, and $\mathscr S\subset\mathscr P(X)$ a subbasis for $\tau$, i.e. $\tau = \tau(\mathscr S)$, then $\mathscr S\subset \tau(\mathscr S)$, since $\tau(\mathscr S)$ is the smallest topology on $X$ containing $\mathscr S$. Hence, $\tau(\mathscr S)$ consists of $\emptyset$, $X$, all finite intersections of members of $\mathscr S$ and arbitrary unions of these finite intersections, and is thus completely determined by $\mathscr S$.
		\end{remark}
